...
@Composable
fun rememberMarker(weightMap: Map<Float, Float> = emptyMap()): Marker {
    ...
    val guideline = lineComponent(
        MaterialTheme.colorScheme.onSurface.copy(alpha = GUIDELINE_ALPHA),
        guidelineThickness,
        guidelineShape,
    )
    return remember(label, indicator, guideline, weightMap) {
        object : MarkerComponent(label, indicator, guideline) {
            init {
                ...
                labelFormatter = MarkerLabelFormatter { markedEntries, _ ->
                    ...
                    if (entry != null) {
                        val weight = weightMap[entry.x]
                        if (weight != null) {
                            String.format("(%.1fkg, %.1f)", weight, entry.y)
                        } else {
            ...
            override fun getInsets(
                ...
                outInsets.top = label.getHeight(context) + labelBackgroundShape.tickSizeDp +
